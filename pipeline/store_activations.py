...
def extract_activations_for_response(model, chat_formatted: str, layer_idx: Optional[int]) -> torch.Tensor:
    """
    Extract activations for a single response using nnsight trace.
    
    Args:
        model: nnsight LanguageModel
        chat_formatted: Chat-formatted text to process
        layer_idx: Specific layer index to extract (None for all layers)
        
    Returns:
        Tensor of activations with shape (num_layers, seq, d_model)
    """
    with model.trace(chat_formatted):
        ...
        activations = torch.stack(layer_outputs, dim=0).save()
...
def store_activations(
    ...
    with torch.no_grad():
        for i, formatted_response in enumerate(responses_to_process):
            ...
            # Extract activations using separate function
            activations = extract_activations_for_response(model, chat_formatted, layer_idx)
